    ...
    while total_games_played < num_games:
        ...
        while True:
            ...
            if move is not None:
                  ...
                  #Update the move on the last element
                  current_game[-1]["move"] = move + 1 #We add 1 to the move to go from 0-8 to 1-9
            else:
                break

            # Check for win or draw
            if check_win(board, current_player, board_size, board_size):
                current_game[-1]["result"] = current_player # We add the result on the last element
                game_data.append(current_game) # We add the current game to the game data
                break
            if check_draw(board):
                current_game[-1]["result"] = "draw"  # We add the result on the last element
                game_data.append(current_game) # We add the current game to the game data
                break
